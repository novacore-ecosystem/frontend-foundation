...
This package MUST NOT depend on React, Vue, Angular, Next.js, Razor, or any UI framework (Tailwind, Shadcn, Radix, MUI, Chakra, Ant Design, etc.) — at build time or runtime. It is the lowest shared layer; future `@novacore/frontend-react`/`-vue`/`-angular` packages will depend on it, never the reverse. Prefer native `Intl`/`Date`/`JSON`/ECMAScript APIs over new dependencies where possible; runtime dependencies are `libphonenumber-js` (phone validation is not solvable with a regex — see `src/phone/index.ts` for the rationale), `axios` (internal transport for `src/http`), and `@microsoft/signalr` (internal transport for `src/realtime`). Axios and SignalR are implementation details, fully owned by this package so consuming apps don't install them separately, and must never appear in `src/http`'s or `src/realtime`'s public exports (no `AxiosRequestConfig`/`AxiosResponse`/`AxiosError`/`HubConnection`/`HubConnectionState` in public types) — see those modules' README sections. This package is not a business API SDK — `src/http` is a generic typed HTTP client/`Endpoint` abstraction, not a wrapper around specific service endpoints — and must not contain domain-specific DTOs (`UserDto`, `OrderDto`, etc.) or domain status enums (`OrderStatus`, `PaymentStatus`, ...); only contracts genuinely shared across every backend service belong here. `src/authorization` provides plain functions (`hasPermission`, `hasAnyPermission`, `hasAllPermissions`) only — no `usePermission()`/`useTranslation()`/`useLocale()` hook or other framework binding belongs in this repo; those are future `@novacore/frontend-react`/`-vue`/`-angular` work.
...
Each domain is its own directory under `src/` (`bootstrap`, `i18n`, `date`, `number`, `currency`, `phone`, `string`, `api`, `validation`, `authorization`, `errors`, `http`, `realtime`, `auth`, `user`, `notifications`) with a barrel `index.ts`; the package's public API is re-exported from `src/index.ts`. `api/` is further split into `response`, `pagination`, `search`, and `error` submodules; `validation/` into `patterns` and `validators`; `authorization/` into `permissions`, `helpers`, `types`, and `translation`; `i18n/` into `locale`, `keys`, `resources` (per-locale, per-namespace files), plus the existing `interpolate`/`resolve`/`translator`; `errors/` into `codes` (re-exports `MessageCode`), `definitions`, and `translation`; `http/` into `types`, `error`, `interceptor`, `configuration`, `endpoint`, `client`, and `token-provider` (a shared `TokenProvider` contract also used by `realtime/configuration`); `realtime/` into `state`, `error`, `event`, `hub`, `configuration`, and `client`; `auth/`, `user/`, and `notifications/` each into `types` + `endpoints` (`EndpointDefinition`s built with `http/endpoint`'s `endpoint()`), with `notifications/` additionally declaring its realtime shape in `hub.ts` via `realtime/hub`'s `hub()` — see those three modules' own doc comments for which contracts are backend-confirmed vs. forward-looking design pending a real audit (`CurrentUserAuthorization`'s `GET user/profiles/current/detail`, and all of `notifications/` — `GET notification/user-notifications/me`, `POST notification/user-notifications/:id/read`, `GET notification/user-notifications/me/unread-count`, and the `GlobalHub`/`ReceiveNotification` realtime push — are confirmed today; see `docs/backend-contract-sync.md`'s 2026-09-22 entry). `http` and `realtime` are also independently importable via the `./http`/`./realtime` package subpath exports (separate `tsup` entries) — Axios and SignalR are internal transports and must never appear in either module's public exports. Do not add catch-all `utils.ts`/`helpers.ts`/`misc.ts` files — every module should represent one coherent capability. Tests mirror this structure under `tests/`.

**Note on `auth`/`user`/`notifications`**: these three modules are a deliberate, narrow exception to the "no business API SDK, no domain-specific DTOs" rule above — authentication, the current user's own profile, and the notification feed are cross-cutting session/identity/platform infrastructure every NovaCore application needs identically (the same category as `TenantBootstrap`/`CurrentUserAuthorization`, already here), not one particular business domain's data (`OrderDto`, `ProductStatus`, etc., which remain out of scope). If a future contract stops being genuinely universal across every NovaCore app, that's a sign it belongs in an application/domain package instead — re-litigate the boundary then, don't assume every future addition here is automatically justified by this precedent.
